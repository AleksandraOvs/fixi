...
if ($is_archive) {
    ...
    foreach ($_cta_devices as $_dev) {
        ...
        if ($_q->have_posts()) {
            $_cta_diag[$_slug] = [];
            while ($_q->have_posts()) { $_q->the_post(); $_cta_diag[$_slug][] = ['title'=>get_the_title(),'url'=>get_permalink(),'slug'=>get_post_field('post_name')]; }
            wp_reset_postdata();
        }
    }
    ...
    <?php if ($is_archive) : ?>
        ...
        document.addEventListener('DOMContentLoaded',function(){
            ...
            if(form){form.addEventListener('submit',function(e){
                ...
                else{alert('Пожалуйста, выберите модель или неисправность');}
            });}
        });
        </script>
            ...
            <div class="container">
                <form class="search-form" method="GET" id="cta-diagnostic-form">
                    ...
                    <select class="search-form__input" id="cta-device-type" name="device" required>
                        ...
                        <?php foreach ($_cta_devices as $_dev): $_slug=sanitize_title($_dev->post_title); if(!empty($_cta_diag[$_slug])): ?>
        ...
        document.addEventListener('DOMContentLoaded',function(){
            ...
            if(devSel){devSel.addEventListener('change',function(){var s=this.value,items=diag[s]||[];diagSel.innerHTML='<option value="" selected>Выберите неисправность</option>';items.forEach(function(d){var o=document.createElement('option');o.value=d.slug;o.textContent=d.title;o.setAttribute('data-device',s);o.setAttribute('data-url',d.url);diagSel.appendChild(o);});});}
            if(form){form.addEventListener('submit',function(e){e.preventDefault();var opt=diagSel.options[diagSel.selectedIndex];var url=opt?opt.getAttribute('data-url'):null;if(url){window.location.href=url;}else{alert('Пожалуйста, выберите устройство и неисправность');}});}
        });
        </script>
        <?php endif; ?>
